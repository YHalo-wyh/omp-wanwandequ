...
Prefer reproducible automation with pwntools/Python. For dynamic evidence use noninteractive gdb/batch commands and preserve exact addresses, offsets, canaries, leaks and libc assumptions. Distinguish PIE-relative offsets from absolute addresses. For heap work, track allocator version, chunk sizes, bin state and safe-linking assumptions explicitly. Do not brute-force ASLR or guess libc when a leak/ELF evidence can resolve it.
